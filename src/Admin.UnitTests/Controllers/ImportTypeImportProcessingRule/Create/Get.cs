...
using ControllerDependencies = Admin.Controllers.ImportTypeImportProcessingRuleControllerDependencies;
using DetailsViewModel = Admin.Models.ImportTypeImportProcessingRule.DetailsViewModel;
...
namespace Admin.UnitTests.Controllers.ImportTypeImportProcessingRule.Create
{
    [TestClass]
    ...
    public class Get
    {
        ...
        {
            var createViewModelBuilder = new Mock<IModelBuilder<EditViewModel, int>>();
            createViewModelBuilder.Setup(x => x.Build()).Returns(new EditViewModel());
            createViewModelBuilder.Setup(x => x.Build(It.IsAny<int>())).Returns(new EditViewModel());
        ...
        public void NavigatablePageActionFilterAttributeHasCorrectDisplayText()
        {
            ...
            Assert.AreEqual("Create Import Processing Rule Link", namedArgument.TypedValue.Value);
        }
