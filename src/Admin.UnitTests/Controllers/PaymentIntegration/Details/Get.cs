...
namespace Admin.UnitTests.Controllers.PaymentIntegration.Details
{
    [TestClass]
    ...
    public class Get
    {
        ...
        {
            var detailsViewModelBuilder = new Mock<IModelBuilder<Models.PaymentIntegration.DetailsViewModel, int>>();
            detailsViewModelBuilder.Setup(x => x.Build(It.IsAny<int>())).Returns(new Models.PaymentIntegration.DetailsViewModel());
        ...
        public void NavigatablePageActionFilterAttributeHasCorrectDisplayText()
        {
            ...
            Assert.AreEqual("Payment Integration Details", namedArgument.TypedValue.Value);
        }
        ...
        public void ReturnsCorrectViewModelType()
        {
            ...
            Assert.IsNotNull(result);
            Assert.IsInstanceOfType(result.Model, typeof(Models.PaymentIntegration.DetailsViewModel));
        }
    }
}
