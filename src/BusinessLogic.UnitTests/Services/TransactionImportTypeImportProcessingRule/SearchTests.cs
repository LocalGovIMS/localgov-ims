...
namespace BusinessLogic.UnitTests.Services.TransactionImportTypeImportProcessingRule
{
    ...
    public class SearchTransactionsTests : BaseTest
    {
        private void SetupUnitOfWork()
        {
            ...
            MockUnitOfWork.Setup(x => x.TransactionImportTypeImportProcessingRules.Search(It.IsAny<SearchCriteria>(), out test))
                .Returns(new List<Entities.TransactionImportTypeImportProcessingRule>()
                    {
                        new Entities.TransactionImportTypeImportProcessingRule()
                        {
                            Id = 1
                        }
                    }
